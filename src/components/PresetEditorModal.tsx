...
export const PresetEditorModal: React.FC<PresetEditorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-scrim/70 backdrop-blur-md animate-pop">
      <div className="w-full max-w-2xl bg-surface border border-line-strong rounded-3xl p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-line">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">자신만의 학급/과목/모둠 맞춤형 템플릿 세트를 저장하세요.</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-muted mb-1 block">템플릿 이름</label>
              <input
                type="text"
                required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-muted mb-1 block">추천 대상 (설명 태그)</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-bold text-muted mb-1 block">템플릿 상세 설명</label>
              <input
                type="text"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-bold text-ink">포함할 역할 목록 ({rolesList.length}개)</h4>
              <button
                type="button"
                onClick={handleAddRoleItem}
                className="flex items-center gap-1 text-xs font-bold px-3 py-1.5 rounded-xl bg-accent/30 hover:bg-accent/50 text-accent-text border border-accent-soft/30"
              >
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
              {rolesList.map((roleItem, idx) => (
                <div key={idx} className="p-4 rounded-2xl bg-elevated/60 border border-line space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <input
                      ...
                      className="bg-elevated text-ink text-xs font-bold px-3 py-1.5 rounded-lg border border-line-strong flex-1"
                    />

                    <div className="flex items-center gap-2">
                      <span className="text-[11px] text-muted">인원:</span>
                      <input
                        type="number"
                        min={1}
                        max={10}
                        ...
                        className="w-14 bg-elevated text-ink text-xs px-2 py-1 rounded-lg border border-line-strong text-center"
                      />
                      ...
                      <select
                        ...
                        className="bg-elevated text-ink text-xs px-2 py-1 rounded-lg border border-line-strong"
                      >
                      ...
                      <button
                        ...
                        className="p-1 text-faint hover:text-rose-400"
                      >
                  ...
                  <input
                    ...
                    className="w-full bg-elevated text-muted text-xs px-3 py-1.5 rounded-lg border border-line-strong"
                  />
                </div>
              ))}
            </div>
